...
void ImGui::ColorConvertFloat4ToRGB(const ImVec4& in, float& out_r, float& out_g, float& out_b)
{
	out_r = (float)((IM_F32_TO_INT8_SAT(in.x)) << IM_COL32_R_SHIFT);
	out_g = (float)((IM_F32_TO_INT8_SAT(in.y)) << IM_COL32_G_SHIFT);
	out_b = (float)((IM_F32_TO_INT8_SAT(in.z)) << IM_COL32_B_SHIFT);
}
...
ImU32 ImGui::FloatArrayGetColorU32(float col[4])
{
	return IM_COL32(int(col[0] * 255), int(col[1] * 255), int(col[2] * 255), int(col[3] * 255));
}

ImU32 ImGui::FloatArrayGetColorU32(ImVec4 col)
{
	return IM_COL32(int(col.z * 255), int(col.y * 255), int(col.x * 255), int(col.w * 255));
}

ImU32 ImGui::GetColorU32(float col[4])
{
	ImGuiStyle& style = GImGui->Style;
	ImVec4 c = ImVec4(col[0], col[1], col[2], col[3]);
	c.w *= 1.f;
	return ColorConvertFloat4ToU32(c);
}
...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
	...
	{
		window->DrawList->AddText(g.Font, g.FontSize, pos + ImVec2(1, 0), 0xFF050505, text, text_display_end);
		window->DrawList->AddText(g.Font, g.FontSize, pos + ImVec2(0, 1), 0xFF050505, text, text_display_end);
		window->DrawList->AddText(g.Font, g.FontSize, pos + ImVec2(-1, 0), 0xFF050505, text, text_display_end);
		window->DrawList->AddText(g.Font, g.FontSize, pos + ImVec2(0, -1), 0xFF050505, text, text_display_end);
...
void ImGui::RenderTextWrapped(ImVec2 pos, const char* text, const char* text_end, float wrap_width)
{
	...
	if (text != text_end)
	{
		window->DrawList->AddText(g.Font, g.FontSize, pos + ImVec2(1, 0), 0xFF050505, text, text_end, wrap_width);
		window->DrawList->AddText(g.Font, g.FontSize, pos + ImVec2(0, 1), 0xFF050505, text, text_end, wrap_width);
		window->DrawList->AddText(g.Font, g.FontSize, pos + ImVec2(-1, 0), 0xFF050505, text, text_end, wrap_width);
		window->DrawList->AddText(g.Font, g.FontSize, pos + ImVec2(0, -1), 0xFF050505, text, text_end, wrap_width);
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	char title[256];
	if (name)
		ImFormatString(title, IM_ARRAYSIZE(title), ("%lld-%s+%08X"), parent_window->Name, name, id);
	else
		ImFormatString(title, IM_ARRAYSIZE(title), ("%lld-%08X"), parent_window->Name, id);
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		if (window_title_visible_elsewhere && !window_just_created && hash_(name) != window->Name)
		{
			size_t buf_len = (size_t)window->NameBufLen;
			window->Name = hash_(name);// ImStrdupcpy(window->Name, &buf_len, name);
...
ImVec2 ImGui::GetNextWindowPos()
{
	ImGuiContext& g = *GImGui;
	return g.CurrentWindow->Pos;
}
...
void ImGui::SameLine(float pos_x, float spacing_w, float pos_y_offset)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	if (pos_x != 0.0f)
	{
		...
		window->DC.CursorPos.y = window->DC.CursorPosPrevLine.y + pos_y_offset;
	}
	else
	{
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, bool override_previous_tooltip)
{
	ImGuiContext& g = *GImGui;
	char window_name[16];
	ImFormatString(window_name, IM_ARRAYSIZE(window_name), ("##+-Tooip+-_%02d"), g.TooltipOverrideCount);
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
	...
	if (extra_flags & ImGuiWindowFlags_ChildMenu)
		ImFormatString(name, IM_ARRAYSIZE(name), ("##+-M.e.n.u_+-%02d"), g.BeginPopupStack.Size); // Recycle windows based on depth
	else
		ImFormatString(name, IM_ARRAYSIZE(name), ("##+-P.o.p.u.p_+-%08x"), id); // Not recycling, so we can close/open during the same frame
...
static std::string GetFallbackWindowNameForWindowingList(ImGuiWindow* window)
{
	if (window->Flags & ImGuiWindowFlags_Popup)
		return ("(P.o.p.u.p)");
	if ((window->Flags & ImGuiWindowFlags_MenuBar) && window->Name == hash_(("##Main+-Menu+-Bar")))
		return ("(M.a.i.n m.e.n.u b.a.r)");
	return ("(U.n.t.i.t.l.e.d)");
}
...
void ImGui::NavUpdateWindowingList()
{
	...
	//    g.NavWindowingList = FindWindowByName(("###Nav+-Window+-ing+-List"));
	//SetNextWindowSizeConstraints(ImVec2(g.IO.DisplaySize.x * 0.20f, g.IO.DisplaySize.y * 0.20f), ImVec2(FLT_MAX, FLT_MAX));
	//SetNextWindowPos(g.IO.DisplaySize * 0.5f, ImGuiCond_Always, ImVec2(0.5f, 0.5f));
	//PushStyleVar(ImGuiStyleVar_WindowPadding, g.Style.WindowPadding * 2.0f);
	//Begin(("###Nav+-Window+-ing+-List"), NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_AlwaysAutoResize/* | ImGuiWindowFlags_NoSavedSettings*/);
	...
	//        label = GetFallbackWindowNameForWindowingList(window).c_str();
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
	...
	struct Funcs
	{
		...
		{
			bool node_open = ImGui::TreeNode(draw_list, ("%s: '%lld' %d vtx, %d indices, %d cmds"), label, draw_list->_OwnerName ? draw_list->_OwnerName : 0, draw_list->VtxBuffer.Size, draw_list->IdxBuffer.Size, draw_list->CmdBuffer.Size);
		...
		static void NodeWindow(ImGuiWindow* window, const char* label)
		{
			...
			ImGui::BulletText(("Flags: 0x%08X (%s%s%s%s%s%s%s%s%s..)"), flags,
				...
				(flags & ImGuiWindowFlags_Modal) ? "Modal " : "", (flags & ImGuiWindowFlags_ChildMenu) ? ("ChildMenu ") : "", ("NoSavedSettings "),
			...
			ImGui::BulletText(("NavLastChildNavWindow: %lld"), window->NavLastChildNavWindow ? window->NavLastChildNavWindow->Name : 0);
	...
	if (ImGui::TreeNode("Popups", "Popups (%d)", g.OpenPopupStack.Size))
	{
		for (int i = 0; i < g.OpenPopupStack.Size; i++)
		{
			...
			ImGui::BulletText("PopupID: %08x, Window: '%lld'%s%s", g.OpenPopupStack[i].PopupId, window ? window->Name : 0, window && (window->Flags & ImGuiWindowFlags_ChildWindow) ? " ChildWindow" : "", window && (window->Flags & ImGuiWindowFlags_ChildMenu) ? " ChildMenu" : "");
		}
		ImGui::TreePop();
	}
	...
	if (ImGui::TreeNode("Internal state"))
	{
		...
		ImGui::Text("HoveredWindow: '%lld'", g.HoveredWindow ? g.HoveredWindow->Name : 0);
		ImGui::Text("HoveredRootWindow: '%lld'", g.HoveredRootWindow ? g.HoveredRootWindow->Name : 0);
		ImGui::Text("HoveredId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d", g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer, g.HoveredIdAllowOverlap); // Data is "in-flight" so depending on when the Metrics window is called we may see current frame information or not
		ImGui::Text("ActiveId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d, Source: %s", g.ActiveId, g.ActiveIdPreviousFrame, g.ActiveIdTimer, g.ActiveIdAllowOverlap, input_source_names[g.ActiveIdSource]);
		ImGui::Text("ActiveIdWindow: '%lld'", g.ActiveIdWindow ? g.ActiveIdWindow->Name : 0);
		ImGui::Text("MovingWindow: '%lld'", g.MovingWindow ? g.MovingWindow->Name : 0);
		ImGui::Text("NavWindow: '%lld'", g.NavWindow ? g.NavWindow->Name : 0);
		ImGui::Text("NavId: 0x%08X, NavLayer: %d", g.NavId, g.NavLayer);
		ImGui::Text("NavInputSource: %s", input_source_names[g.NavInputSource]);
		ImGui::Text("NavActive: %d, NavVisible: %d", g.IO.NavActive, g.IO.NavVisible);
		ImGui::Text("NavActivateId: 0x%08X, NavInputId: 0x%08X", g.NavActivateId, g.NavInputId);
		ImGui::Text("NavDisableHighlight: %d, NavDisableMouseHover: %d", g.NavDisableHighlight, g.NavDisableMouseHover);
		ImGui::Text("NavWindowingTarget: '%lld'", g.NavWindowingTarget ? g.NavWindowingTarget->Name : 0);
