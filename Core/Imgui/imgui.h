...
#define IMGUI_VERSION               "1"
#define IMGUI_VERSION_NUM           160899
...
struct ImVec2
{
	float     x, y;
	ImVec2() { x = y = 0.0f; }
	ImVec2(float value) { x = value; y = value; }
	ImVec2(float _x, float _y) { x = _x; y = _y; }
	ImVec2(const IronMan::Core::Math::FVector2D& vec) { x = vec.X; y = vec.Y; }
...
struct ImVec4
{
	...
	ImVec4(float* data) { x = data[0]; y = data[1]; z = data[2]; w = data[3]; }
...
namespace ImGui
{
	...
	IMGUI_API ImVec2        GetNextWindowPos();
	...
	IMGUI_API ImU32         GetColorU32(float col[4]);                                      // retrieve given color with style alpha applied
	IMGUI_API ImU32         GetColorU32(ImU32 col);                                         // retrieve given color with style alpha applied
	IMGUI_API ImU32         FloatArrayGetColorU32(float col[4]);
	IMGUI_API ImU32         FloatArrayGetColorU32(ImVec4 col);
	...
	IMGUI_API void          SameLine(float local_pos_x = 0.0f, float spacing_w = -1.0f, float pos_y_offset = 0.0f);    // call between widgets or groups to layout them horizontally. X position given in window coordinates.
	...
	IMGUI_API void          TextUnformatted2(const char* text, const char* text_end = NULL);                // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
	...
	IMGUI_API bool          Checkbox1(const char* label, int* v, float* Color);
	IMGUI_API bool          Checkbox2(const char* label, int* v);
	IMGUI_API bool          ToggleButton(const char* str_id, bool* v, float x_offset = 0.0f, float y_offset = 0.0f, float text_spacing_offset = 5.0f);
	...
	// - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
	IMGUI_API bool          SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", float power = 1.0f, bool decvalue = true);     // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display. Use power!=1.0 for power curve sliders
	IMGUI_API bool          SliderFloat2(const char* label, float v[2], float v_min, float v_max, const char* format = "%.3f", float power = 1.0f);
	IMGUI_API bool          SliderFloat3(const char* label, float v[3], float v_min, float v_max, const char* format = "%.3f", float power = 1.0f);
	IMGUI_API bool          SliderFloat4(const char* label, float v[4], float v_min, float v_max, const char* format = "%.3f", float power = 1.0f);
	IMGUI_API bool          SliderAngle(const char* label, float* v_rad, float v_degrees_min = -360.0f, float v_degrees_max = +360.0f, const char* format = "%.0f deg");
	IMGUI_API bool          SliderInt(const char* label, int* v, int v_min, int v_max, const char* format = "%d", bool decvalue = true);
	IMGUI_API bool          SliderInt2(const char* label, int v[2], int v_min, int v_max, const char* format = "%d");
	IMGUI_API bool          SliderInt3(const char* label, int v[3], int v_min, int v_max, const char* format = "%d");
	IMGUI_API bool          SliderInt4(const char* label, int v[4], int v_min, int v_max, const char* format = "%d");
	IMGUI_API bool          SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format = NULL, float power = 1.0f, bool decvalue = true);
	IMGUI_API bool          SliderScalarN(const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max, const char* format = NULL, float power = 1.0f);
	IMGUI_API bool          VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* format = "%.3f", float power = 1.0f);
	IMGUI_API bool          VSliderInt(const char* label, const ImVec2& size, int* v, int v_min, int v_max, const char* format = "%d");
	IMGUI_API bool          VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format = NULL, float power = 1.0f);

	IMGUI_API void          RenderTextOverlay(ImVec2 pos, const char* text, const char* text_end = NULL, bool hide_text_after_hash = true);
	// Widgets: Input with Keyboard
	// - If you want to use InputText() with a dynamic string type such as std::string or your own, see misc/cpp/imgui_stdlib.h
	// - Most of the ImGuiInputTextFlags flags are only useful for InputText() and not for InputFloatX, InputIntX, InputDouble etc.
	// ImGui::InputText() with std::string
	// Because text input needs dynamic resizing, we need to setup a callback to grow the capacity
	IMGUI_API bool			InputText(const char* label, std::string* str, size_t lenght = 0, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
	...
	IMGUI_API void         ColorConvertFloat4ToRGB(const ImVec4& in, float& out_r, float& out_g, float& out_b);
...
struct ImColor
{
	...
	ImColor(float col[4]) { Value.x = col[0]; Value.y = col[1]; Value.z = col[2]; Value.w = col[3]; }
...
struct ImDrawList
{
	...
	hash_t _OwnerName;         // Pointer to owner window's name for debugging
	...
	// Primitives
	IMGUI_API void  DrawBox_S(const ImVec2& pos, const ImVec2& headPosition, uint32_t color, float Div, float Siz);
	IMGUI_API void  DrawHealthBar(const ImVec2& pos, const ImVec2& headPosition, float Health, float HealthMax);
	...
	IMGUI_API ImVec2 DrawTextOutline(const ImFont* font, float fontSize, float clip_w, ImVec2 pos, ImU32 col, ImU32 outlineCol, DWORD alignment, const char* text, ...);
