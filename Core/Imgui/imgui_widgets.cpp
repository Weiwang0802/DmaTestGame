...
void ImGui::TextUnformatted2(const char* text, const char* text_end)
{
	...
	else
	{
		...
		// Render (we don't hide text after ## in this end-user function)
		RenderTextWrapped(ImVec2(bb.Max.x, bb.Min.y), text_begin, text_end, wrap_width);
	}
}
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
	...
	const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
	RenderNavHighlight(bb, id);
	RenderFrame(bb.Min, bb.Max, 0, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, style.FrameRounding));
	if (bg_col.w > 0.0f)
...
bool ImGui::Checkbox1(const char* label, int* v, float* Color)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 2 + 16, label_size.y + style.FramePadding.y * 2)); // We want a square shape to we use Y twice
	...
	if (*v)
	{
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y + 10.25), 5.6f, GetColorU32(Color), 12);
	}
	else
	{
		/*float RevColor[4];
		...
		RevColor[2] = Color[1];
		RevColor[3] = Color[3];
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y + 10.25), 5.6f, GetColorU32(RevColor), 12);*/
		window->DrawList->AddCircle(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y + 10.25), 5.6f, GetColorU32(Color), 12);
	}
	if (label_size.x > 0.0f)
	{
		ImVec2 T = text_bb.GetTL();
		T.y -= 2.f;
		T.x += -12.f;
		RenderText(T, label);
	}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (*v)
	{
		...
		RenderCheckMark(check_bb.Min + ImVec2(pad, pad), GetColorU32(ImVec4(1.00f, 0.79f, 0.18f, 0.78f)), square_sz - pad * 2.0f);
	}
...
bool ImGui::Checkbox2(const char* label, int* v)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const float check_sz2 = check_sz / 2;
	const float pad = ImMax(1.0f, (float)(int)(check_sz / 4.f));
	auto ActiveColor = GetColorU32(ImVec4(0.8549f, 0.3686f, 0.3686f, 1.0f));
	auto UnActiveColor = GetColorU32(ImVec4(0.4039f, 0.5725f, 0.4039f, 1.0f));
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 6, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 5, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 4, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 3, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 2, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 1, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 1, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 2, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 3, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 4, check_bb.Min.y + 9), 7, UnActiveColor, 12);
	...
	window->DrawList->AddCircle(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 6, check_bb.Min.y + 9), 7, GetColorU32(ImVec4(0.39f, 0.39f, 0.39f, 1.0f)), 12);
	if (*v)
	{
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 6, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 5, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 4, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 3, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 2, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 1, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 1, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 2, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 3, check_bb.Min.y + 9), 7, ActiveColor, 12);
		window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 4, check_bb.Min.y + 9), 7, ActiveColor, 12);
	...
	if (label_size.x > 0.0f)
		RenderText(text_bb.GetTL() - ImVec2(0, 3), label);
	return pressed;
}
bool ImGui::ToggleButton(const char* label, bool* v, float x_offset, float y_offset, float text_spacing_offset)
{
	...
	const float width = square_sz * 1.70f;
	const float radius = square_sz * 0.50f;
	const ImRect check_bb(pos, pos + ImVec2(width, square_sz));
	const ImRect total_bb(pos, pos + ImVec2(width + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + text_spacing_offset + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, id))
		return false;
	...
	float t = *v ? 1.0f : 0.0f;
	constexpr float ANIM_SPEED = 0.1f;

	if (held && hovered) g.LastActiveIdTimer = 0;

	if (g.LastActiveId == window->GetID(label) && !held) // && g.LastActiveIdTimer < ANIM_SPEED)
	{
		float t_anim = ImSaturate(g.LastActiveIdTimer / ANIM_SPEED);
		t = *v ? (t_anim) : (1.0f - t_anim);
	}

	window->DrawList->AddRectFilled(pos, ImVec2(pos.x + width, pos.y + square_sz),
		GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), radius);

	const auto center = ImVec2(pos.x + radius, pos.y + radius);
	window->DrawList->AddCircleFilled(center, radius, IM_COL32(199, 79, 13, 255), 16);
	const auto backgroup_max = ImVec2(pos.x + t * radius * 2.0f + 2.0f, pos.y + square_sz);
	if (backgroup_max.x > pos.x + radius)
	{
		window->DrawList->AddRectFilled(ImVec2(pos.x + radius, pos.y), backgroup_max, IM_COL32(199, 79, 13, 255));
	}
	window->DrawList->AddCircleFilled(ImVec2(center.x + t * (width - radius * 2.0f), center.y), radius, IM_COL32(255, 255, 255, 255), 16);

	/*if (g.LogEnabled)
		LogRenderedText(&total_bb.Min, *v ? ("[x]") : ("[ ]"));*/
	if (label_size.x > 0.0f)
		RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + text_spacing_offset, check_bb.Min.y + style.FramePadding.y - 1.0f), label);
	return pressed;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	if (!(flags & ImGuiComboFlags_NoPreview))
		window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x + 2, frame_bb.Max.y), frame_col, 4.f);
	if (!(flags & ImGuiComboFlags_NoArrowButton))
	{
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max + ImVec2(3.f, 0), GetColorU32((popup_open || hovered) ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 4.f, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
		RenderArrow(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), ImGuiDir_Down);
	}
	RenderFrameBorder(frame_bb.Min, frame_bb.Max, style.FrameRounding);
	//默认显示的文本
	if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
		RenderTextClipped(frame_bb.Min + style.FramePadding, value_bb.Max - ImVec2(0, 2), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
	//后面的标签
	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x + 2, frame_bb.Min.y + style.ItemInnerSpacing.y - 2), label);
	...
	char name[16];
	ImFormatString(name, IM_ARRAYSIZE(name), "##+-C.o.m.b.o_%02d", g.BeginPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
	...
	if (!BeginCombo(label, preview_value, ImGuiComboFlags_HeightRegular))
		return false;
	...
	for (int i = 0; i < items_count; i++)
	{
		...
		if (!item_text || IsBadReadPtr(item_text, 1))
		{
			PopID();
			break;
		}
...
static bool DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* format)
{
	...
	if (format == NULL)
		format = GDataTypeInfo[data_type].ScanFmt.c_str();
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	if (format == NULL)
		format = GDataTypeInfo[data_type].PrintFmt.c_str();
...
void ImGui::RenderTextOverlay(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
	...
	{
		ImGui::GetWindowDrawList()->AddText(g.Font, g.FontSize, pos + ImVec2(1, 0), ImGui::GetColorU32(ImVec4(0, 0, 0, 1)), text, text_display_end);
		ImGui::GetWindowDrawList()->AddText(g.Font, g.FontSize, pos + ImVec2(0, 1), ImGui::GetColorU32(ImVec4(0, 0, 0, 1)), text, text_display_end);
		ImGui::GetWindowDrawList()->AddText(g.Font, g.FontSize, pos + ImVec2(-1, 0), ImGui::GetColorU32(ImVec4(0, 0, 0, 1)), text, text_display_end);
		ImGui::GetWindowDrawList()->AddText(g.Font, g.FontSize, pos + ImVec2(0, -1), ImGui::GetColorU32(ImVec4(0, 0, 0, 1)), text, text_display_end);
		ImGui::GetWindowDrawList()->AddText(g.Font, g.FontSize, pos, ImGui::GetColorU32(ImGuiCol_Text), text, text_display_end);
	}
}

bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, bool decvalue)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(210.f / 255.f, 200.f / 255.f, 120.f / 255.f, 1.f));
	RenderTextOverlay(ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x - 6.f, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(16, -9), value_buf[0], value_buf_end[0]);
	ImGui::PopStyleColor();

	RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 2), ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 2), ImColor(168, 168, 168), true, 2.f);
	RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 2), ImVec2(grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 2), ImColor(210, 200, 120), true, 2.f);
	window->DrawList->AddRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 2), ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 2), ImColor(150, 150, 150, 255));

	window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f) + 2.5), 5.f, ImColor(255, 255, 255, 255), 64);
	window->DrawList->AddCircle(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f) + 2.5), 5.f, ImColor(100, 100, 100, 255), 64);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, float power, bool decvalue)
{
	return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, power, decvalue);
}
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* format, bool decvalue)
{
	return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format, decvalue);
}
...
bool ImGui::InputText(const char* label, std::string* str, size_t lenght, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
	...
	return InputText(label, (char*)str->c_str(), lenght == 0 ? (str->capacity() + 1) : lenght, flags, InputTextCallback, &cb_user_data);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		static std::string ids[4] = { ("##X"), ("##Y"), ("##Z"), ("##W") };
		static std::string fmt_table_int[3][4] =
		{
			{   ("%3d"),   ("%3d"),   ("%3d"),   ("%3d")}, // Short display
			{ ("R:%3d"), ("G:%3d"), ("B:%3d"), ("A:%3d")}, // Long display for RGBA
			{ ("H:%3d"), ("S:%3d"), ("V:%3d"), ("A:%3d")}  // Long display for HSVA
		};
		static std::string fmt_table_float[3][4] =
		{
		...
		for (int n = 0; n < components; n++)
		{
			...
			{
				value_changed |= DragFloat(ids[n].c_str(), &f[n], 1.0f / 255.0f, 0.0f, hdr ? 0.0f : 1.0f, fmt_table_float[fmt_idx][n].c_str());
				value_changed_as_float |= value_changed;
			}
			else
			{
				value_changed |= DragInt(ids[n].c_str(), &i[n], 1.0f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][n].c_str());
			}
			if (!(flags & ImGuiColorEditFlags_NoOptions))
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
	{
		...
		if (ColorButton(("##+-ColorButton"), col_v4, flags, ImVec2(0, 0)))
		{
	...
	{
		SameLine(0, style.ItemInnerSpacing.x);
		TextUnformatted2(label, label_display_end);
	}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview))
	{
		...
		if ((flags & ImGuiColorEditFlags_NoLabel))
			Text(("###Current###"));
		ColorButton(("##+-current"), col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 2, square_sz * 2));
		if (ref_col != NULL)
		{
			Text(("###Original###"));
			ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
			if (ColorButton(("##+-original"), ref_col_v4, (flags & (ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoTooltip)), ImVec2(square_sz * 2, square_sz * 2)))
			{
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
	...
	if (size.y == 0.0f)
		size.y = default_size;
	const ImRect bb(window->DC.CursorPos + ImVec2(30.f, 0), window->DC.CursorPos + size + ImVec2(30.f, 0));
	ItemSize(bb, (size.y >= default_size) ? g.Style.FramePadding.y : 0.0f);
	if (!ItemAdd(bb, id))
		return false;
	...
	else
	{
		...
		if (col_source.w < 1.0f)
			RenderColorRectWithAlphaCheckerboard(ImVec2(bb_inner.Min.x, bb_inner.Min.y + 6), ImVec2(bb_inner.Max.x, bb_inner.Max.y - 6), GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
		else
		{
			window->DrawList->AddRectFilled(ImVec2(bb_inner.Min.x + 4, bb_inner.Min.y + 4), ImVec2(bb_inner.Max.x - 4, bb_inner.Max.y - 4), GetColorU32(col_source), rounding, ImDrawCornerFlags_All);
			window->DrawList->AddRect(ImVec2(bb_inner.Min.x + 2, bb_inner.Min.y + 2), ImVec2(bb_inner.Max.x - 2, bb_inner.Max.y - 2), 0xFFFFFFFF, 2.f, ImDrawCornerFlags_All, 1);
		}
	}
	RenderNavHighlight(bb, id);
	...
	//   else
	   //{
	   //	//window->DrawList->AddRect(ImVec2(bb.Min.x, bb.Min.y + 6), ImVec2(bb.Max.x, bb.Max.y - 6), GetColorU32(ImGuiCol_FrameBg), rounding); // Color button are often in need of some sort of border
	   //	window->DrawList->AddRect(ImVec2(bb.Min.x, bb.Min.y + 6), ImVec2(bb.Max.x, bb.Max.y - 6), IM_COL32(80, 120, 228, 255), rounding); // Color button are often in need of some sort of border
...
bool ImGui::BufferingBar(const char* label, float value, const ImVec2& size_arg, const ImU32& bg_col, const ImU32& fg_col) {
	...
	// Render
	const float rectWidth = size.x;
	const float circleWidth = rectWidth * value;

	const float t = g.Time;
	const float r = size.y / 2;
	const float speed = 1.5;

	window->DrawList->AddRectFilled(bb.Min, ImVec2(pos.x + rectWidth, bb.Max.y), bg_col, r);
	window->DrawList->AddRectFilled(bb.Min, ImVec2(pos.x + rectWidth * value, bb.Max.y), fg_col, r);
	...
	const float o1 = (circleWidth - r - 2) * (t + a - speed * (int)((t + a) / speed)) / speed;
	const float o2 = (circleWidth - r - 2) * (t + b - speed * (int)((t + b) / speed)) / speed;
	const float o3 = (circleWidth - r - 2) * (t + c - speed * (int)((t + c) / speed)) / speed;

	window->DrawList->AddCircleFilled(ImVec2(pos.x + r + o1, bb.Min.y + r), r, IM_COL32(255, 255, 255, 255));
	window->DrawList->AddCircleFilled(ImVec2(pos.x + r + o2, bb.Min.y + r), r, IM_COL32(255, 255, 255, 255));
	window->DrawList->AddCircleFilled(ImVec2(pos.x + r + o3, bb.Min.y + r), r, IM_COL32(255, 255, 255, 255));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (hovered || selected)
	{
		const ImU32 col = GetColorU32(((held && hovered) || selected) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
bool ImGui::BeginMainMenuBar()
{
	...
	bool is_open = Begin(("##Main+-Menu+-Bar"), NULL, window_flags) && BeginMenuBar();
	PopStyleVar(2);
