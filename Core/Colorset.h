...
#define DEFCOLOR_SRC(name, r, g, b) \
	static Colorset name##(uint8_t a = 255){ return Colorset(r, g, b, a); }
...
public:
	...
	Colorset(uint8_t r, uint8_t g, uint8_t b) {
		SetColor(r, g, b, 255);
	}

	Colorset(uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
		SetColor(r, g, b, a);
	}
	...
	Colorset DiffAlpha(int a)
	{
		return Colorset(this->r(), this->g(), this->b(), a);
	}
	...
	Colorset operator+(const Colorset& rhs) const {
		...
		return Colorset(red, green, blue, alpha);
	}
	...
	static Colorset Outline()
	{
		return Colorset(9, 9, 9);
	}

	static Colorset FromHSB(float hue, float saturation, float brightness)
	{
