...
namespace IronMan
{
	...
	bool Zydis::Disassemble(size_t addr, const unsigned char* data, int size)
	{
		...
		/*if (!ZYAN_SUCCESS(ZydisFormatterFormatInstruction(
			...
			mInstrText,
			sizeof(mInstrText), m_RuntimeAddress)))
			return false;*/

			//CONSOLE_INFO("%016" PRIX64 "	%s", m_RuntimeAddress, &mInstrText[0]);
	...
	bool Zydis::DisassembleSafe(size_t addr, const unsigned char* data, int size)
	{
		...
		memcpy(dataSafe, data, std::min(MAX_DISASM_BUFFER, size));
	...
	std::string Zydis::OperandText(int opindex) const
	{
		...
		case ZYDIS_OPERAND_TYPE_IMMEDIATE:
			type = ZYDIS_FORMATTER_FUNC_FORMAT_OPERAND_IMM;
			break;
		case ZYDIS_OPERAND_TYPE_MEMORY:
			type = ZYDIS_FORMATTER_FUNC_FORMAT_OPERAND_MEM;
			break;
		case ZYDIS_OPERAND_TYPE_REGISTER:
			type = ZYDIS_FORMATTER_FUNC_FORMAT_OPERAND_REG;
			break;
		case ZYDIS_OPERAND_TYPE_POINTER:
			type = ZYDIS_FORMATTER_FUNC_FORMAT_OPERAND_PTR;
			break;
		default:
			return "";
		}
		...
		ZydisFormatterFunc fmtFunc = nullptr;
		if (!ZYAN_SUCCESS(ZydisFormatterSetHook(&mFormatter, type, (const void**)&fmtFunc)))
			return "";


		ZydisFormatterContext context;
		context.instruction = &mInstr;
		context.runtime_address = m_RuntimeAddress;
		context.operand = &op;
		...
		formatter_buffer.is_token_list = ZYAN_FALSE;
		formatter_buffer.string.flags = ZYAN_STRING_HAS_FIXED_CAPACITY;
		formatter_buffer.string.vector.allocator = nullptr;
		formatter_buffer.string.vector.element_size = sizeof(char);
		formatter_buffer.string.vector.size = 1;
		formatter_buffer.string.vector.capacity = sizeof(buf) - 1;
		formatter_buffer.string.vector.data = buf;
	...
	const ZydisDecodedOperand& Zydis::operator[](int index) const
	{
		...
		{
			CONSOLE_INFO("%d|%d|%d", Success(), index, OpCount());
			DebugBreak();
		}
