...
namespace IronMan
{
	...
	{
	public:
		AsmStackAllocator(asmjit::X86Assembler* pAsm, int32_t baseval = 0x28)
			: _pAsm(pAsm)
		...
		asmjit::Mem AllocVar(int32_t size, bool align = true)
		{
		...
		bool AllocArray(asmjit::Mem arr[], int count, int32_t size)
		{
	...
	#define ALLOC_STACK_VAR_S_NOALIGN(worker, name, size) asmjit::Mem name( worker.AllocVar( size,false ) );
